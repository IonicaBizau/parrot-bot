...
class ParrotBot extends EventEmitter {
    ...
    getFailMessage () {
        let message = this.config.messages.fail[
            Math.floor(Math.random() * this.config.messages.fail.length)
        ];
        return message;
    }

    getDuplicateMessage (type) {
        let duplicateMessages = this.config.messages.duplicate;
        Logger.log(duplicateMessages);
        let arr = duplicateMessages[type];
...
/* NOT DUPPLICATED! ->*/if (words.indexOf(keyword) === -1 && this.config.meta.ignore.indexOf(keyword) === -1) {
            ...
            this.processMessageToSend(message, (err, message) => {
                ...
                if (!message) {
                    collection.find({ "type": "Q"}).toArray((err, docs) => {
    ...
    */
    ...
    processMessageToSend (message, cb) {
        ...
        if (!message || !words || !words.length) {
            ...
            collection.find({ "type": "Q" }).toArray((err, docs) => {
                ...
                if (this.useCache) { this.cache.sent["Q"].push(randQuestion); }
    ...
    duplicateMemory (colOne, colTwo, cb) {
        ...
        this.db.collection(colOne, (err, col) => {
            if (err) { return cb(err); }
            ...
            col.find().toArray((err, data) => {
                ...
                this.db.collection(colTwo, (err, col) => {
                    if (err) { return cb(err); }
